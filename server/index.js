...
const corsOptions = {
  ...
  methods: 'GET,POST,PUT,DELETE,OPTIONS',             // Only allow GET and POST requests
...
app.post("/api/login", (_req, res) => {
  ...
  db.all(sql, [email, password], (err, results) => {
    if (err) {
...
app.post("/api/post/employees",upload.single("image"),(req, res) => {
  ...
  db.run(sql, [name, position, salary, hireDate, status, imagePath, phone, birthday], (err, result) => {
    if (err) {
...
app.post("/api/create/agent",upload.single("image"),(req, res) => {
  ...
  db.run(sql, [name, email, role, password, phone, birthday, imagePath], (err, result) => {
    if (err) {
...
app.delete("/api/delete/agent/:id", (req, res) => {
  ...
  db.run(sqlSelect, [id], (err, results) => {
...
app.post("/api/client/create", upload.single("image"), (req, res) => {
  ...
  db.beginTransaction((err) => {
    ...
    db.run(sqlClient, [name, phone, address, gender, email, subscriptionDuration, amount, formattedStart, formattedEnd, nationality, birthday, status, imagePath, agent,subscription], (err1, result) => {
      if (err1) {
      ...
      db.run(sqlGestion, ['Income', 'Abonnements', amount, formattedStart, `Abonnement pour ${name}`], (err2) => {
        if (err2) {
...
app.post("/api/saveExpense", (req, res) => {
  const { type, category, amount, date,description } = req.body;
    ...
    db.run(sql, [type, category, amount, date,description], (err) => {
      if (err) {
        res.json({
...
app.post("/api/inventory", (req, res) => {
  const { name, category, quantity, minQuantity, unitPrice,lastRestockDate } = req.body;
    ...
    db.run(sql, [name, category, quantity, minQuantity, unitPrice,lastRestockDate], (err) => {
      if (err) {
        res.json({
...
app.post("/api/inventory/sell", (req, res) => {
  ...
  db.run(checkStockSql, [idNum], (err, results) => {
    if (err) {
    ...
    db.run(updateSql, [qty, idNum], (err, updateResult) => {
      if (err) {
      ...
      db.run(insertTx, ['Income', 'Vente', price, date, itemName], (err2) => {
        if (err2) {
...
app.post("/api/inventory/restock", (req, res) => {
  ...
  const myDate = new Date().toISOString().split('T')[0]; // YYYY-MM-DD format
    ...
    db.run(updateSql, [name, category, unitPrice, qty, minQuantity, myDate, idNum], (err, updateResult) => {
      if (err) {
        return res.status(500).json({ error: 'Erreur lors de la mise à jour du stock' });
      }
